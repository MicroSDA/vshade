...
void main()
{
    gl_Position        = u_camera.view_projection_matrix * s_transforms[gl_InstanceIndex] * vec4(a_position, 1.0);
...
void main()
{
    ...
    out_fragment_color = vec4(texture(MaterialTextures[albedo_texture_index], in_uv_coordinates).rgb * s_material[in_instance_id].ambient_color,
                              texture(MaterialTextures[albedo_texture_index], in_uv_coordinates).a);
}