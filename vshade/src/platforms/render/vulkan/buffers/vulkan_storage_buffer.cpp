...
vshade::render::VulkanStorageBuffer::VulkanStorageBuffer(BufferUsage const usage, std::uint32_t const binding_index, std::uint32_t const data_size,
                                                         std::uint32_t const count, std::uint32_t const resize_threshold)
    : StorageBuffer(usage, binding_index, data_size, count, resize_threshold)
{
    invalidate(usage, binding_index, data_size, count, resize_threshold);
}

vshade::render::VulkanStorageBuffer::~VulkanStorageBuffer()
{
...
void vshade::render::VulkanStorageBuffer::invalidate(BufferUsage const usage, std::uint32_t const binding_index, std::uint32_t const data_size,
                                                     std::uint32_t const count, std::uint32_t resize_threshold)
{
    ...
    vk_buffer_create_info.usage       = VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_TRANSFER_SRC_BIT;
...
void vshade::render::VulkanStorageBuffer::setData(std::uint32_t const size, void const* data_ptr, std::uint32_t const frame_index,
...
void vshade::render::VulkanStorageBuffer::resize(std::uint32_t const size)
{
...
std::tuple<VkBuffer, VkDeviceMemory> vshade::render::VulkanStorageBuffer::createVkBuffer(VulkanInstance vulkan_instance, VkDevice vk_logical_device,
                                                                                         ...
                                                                                         VkPhysicalDevice             vk_physical_device)
{
    ...
    vk_utils::setDebugObjectName<VK_OBJECT_TYPE_BUFFER>(vulkan_instance.instance, "Vulkan storage buffer", vk_logical_device, vk_buffer);
    ...
    vk_utils::setDebugObjectName<VK_OBJECT_TYPE_DEVICE_MEMORY>(vulkan_instance.instance, "Vulkan storage buffer memory", vk_logical_device,
                                                               vk_buffer_memory);
...
std::uint32_t vshade::render::VulkanStorageBuffer::getBufferMinAlignment(std::size_t const size) const
{
    ...
    std::uint32_t const max_buffer_size{vulkan_physical_device->getPhysicalDeviceProperties().limits.maxStorageBufferRange};
    if (size > max_buffer_size)
    {
        VSHADE_CORE_ERROR("Storage buffer size {} is biger then VkPhysicalDeviceLimits::maxStorageBufferRange {}", size, max_buffer_size);
    }

    // Calculate required alignment based on minimum device offset alignment
    std::size_t const min_sbo_alignment{vulkan_physical_device->getPhysicalDeviceProperties().limits.minStorageBufferOffsetAlignment};
    std::size_t       aligned_size{size};

    if (min_sbo_alignment > 0U)
    {
        aligned_size = (aligned_size + min_sbo_alignment - 1U) & ~(min_sbo_alignment - 1U);
    }
